...
-- A rate distinguishes an individual lesson from a package lesson. The teacher
-- payout is a percentage of the price actually charged for that lesson.
create table public.student_rates (
  ...
  package_lesson_price_uah integer not null check (package_lesson_price_uah >= 0),
...
-- A prepaid credit reserves the exact lesson price active on the factual
-- payment date. Its unused value is excluded from the student's free balance.
create table public.student_payment_credits (
  id uuid primary key default gen_random_uuid(),
  school_id uuid not null references public.schools(id) on delete cascade,
  student_id uuid not null references auth.users(id) on delete restrict,
...
alter table public.wallet_ledger
  add column payment_credit_id uuid references public.student_payment_credits(id) on delete restrict;
...
create index student_payment_credits_available_idx on public.student_payment_credits (school_id, student_id, paid_at, created_at) where remaining_lesson_count > 0;
...
create policy "payment_credits_admin_read" on public.student_payment_credits for select to authenticated using (public.is_school_admin(school_id));
...
-- An admin records the received sum and the number of prepaid lessons. The
-- server combines the new sum with only unreserved money and issues credits
-- only when the selected quantity is fully funded.
